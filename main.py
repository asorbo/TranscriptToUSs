...
async def segment_transcript(transcript, n_runs=10):
    ...
    async def safe_generate():
        try:
            logger.info("Started a segmentation prompt")
    ...
    logger.info("obtained " + str(len(runs)) + " segmentations.")
    return runs
...
async def infer_missing_roles(topic_texts, roles):
    ...
    async def safe_generate_inferred_role(req_info):
        try:
            ...
            if isinstance(result, list):
                ...
                logger.error(f"Unexpected LLM output: {result}")
        except json.JSONDecodeError as e:
            logger.error(f"JSONDecodeError: {e}")
    ...
    for change in inferred_roles:
        try:
            ...
            logger.error(f"The requirement_id included in a change generated by the LLM is incorrect and does not match any requirement. The following change could not be applied:\n{str(change)}\nException: {e}")
            continue
...
async def infer_missing_rationales(topic_texts, roles):
    ...
    for change in inferred_rationales:
        ...
        try:
            ...
            logger.error("Error: the requirement_id included in a change generated by the LLM is incorrect and does not match any requirement. The following change could not be applied:\n" + str(change))
            continue
    ...
    for req in requirements_set:
        ...
        if user_story:
            try:
                ...
                logger.error(f"Error in the generation for {req.requirement_id}: {e}")
...
async def check_set_level_violations(requirements_set):
    ...
    if set_level_violations:
        logger.info(f"{len(set_level_violations)} set level violations found.")
    else:
        logger.info("No set level violations found.")
...
async def run_pipeline(transcript, stop_event):
    ...
    # Segment transcript
    logger.info("Part 1/10 started: segment transcript")
    runs = await segment_transcript(transcript, n_runs=5)
    if not runs:
        logger.error("Failed to segment transcript.")
        return
    logger.info("Part 1/10 completed: segment transcript")

    if is_stop_requested(stop_event):
        return
    
    # Select runs with the most frequent number of topics
    logger.info("Part 2/10 started: select runs with number of topics = mode")
    runs_with_same_amount_topics = get_runs_with_same_amount_topics(runs)
    if not runs_with_same_amount_topics:
        logger.error("No runs found with the most frequent number of topics.")
        return
    logger.info("Part 2/10 completed: select runs with number of topics = mode")
    ...
    logger.info("Part 3/10 started: Get topic texts and speaker turns")
    topic_texts = await get_topic_texts(selected_topic_list, transcript)
    if not topic_texts:
        logger.error("Failed to get topic texts.")
        return
    output['topic_texts'] = topic_texts
    logger.info("Part 3/10 completed: Get topic texts and speaker turns")

    if is_stop_requested(stop_event):
        return
    
    # Identify roles
    logger.info("Part 4/10 started: Identify roles")
    ...
    logger.info("Part 4/10 completed: Identify roles")

    if is_stop_requested(stop_event):
        return

    # Extract requirements
    logger.info("Part 5/10 started: extract requirements")
    topic_texts_with_requirements = await extract_requirements(topic_texts, roles)
    logger.info("Part 5/10 completed: extract requirements")

    if is_stop_requested(stop_event):
        return

    # Infer missing roles
    logger.info("Part 6/10 started: Infer missing roles")
    topic_texts_with_inferred_roles = await infer_missing_roles(topic_texts_with_requirements, roles)
    logger.info("Part 6/10 completed: Infer missing roles")

    if is_stop_requested(stop_event):
        return
    
    # Infer missing rationales
    logger.info("Part 7/10 started: Infer missing rationales")
    topic_texts_with_inferred_rationales = await infer_missing_rationales(topic_texts_with_inferred_roles, roles)
    logger.info("Part 7/10 completed: Infer missing rationales")

    if is_stop_requested(stop_event):
        return

    # Build Requirement objects
    logger.info("Part 8/10 started: Generate set of Requirement objects")
    requirements_set = build_requirements_set(topic_texts_with_inferred_rationales)
    logger.info("Part 8/10 completed: Generate set of Requirement objects")

    if is_stop_requested(stop_event):
        return
    
    # Check criteria violations for each requirement (optional, call if needed)
    logger.info("Part 9/10 started: Check criteria violations for each requirement")
    requirements_set = await check_criteria_violations(requirements_set)
    logger.info("Part 9/10 completed: Check criteria violations for each requirement")
...
def stop_logging_stream():
    ...
    log_queue.put(done_message)

def is_stop_requested(stop_event):
...
class QueueHandler(logging.Handler):
    def __init__(self, log_queue):
        super().__init__()
        self.log_queue = log_queue  # store reference to the queue

    def emit(self, record):
        log_entry = self.format(record)
        self.log_queue.put(log_entry)  # put log into the passed queue

# This function configures logging and attaches the QueueHandler
def configure_logs(log_queue):
    global logger
